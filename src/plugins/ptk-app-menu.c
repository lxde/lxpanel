...
/* NOTE:
        This is a dirty hack to save memory and execution time.
        Normally this value should be generated by g_quark_from_static_string,
        but since we only uses the value in this file, we can use
        GPOINTER_TO_UINT because the addresses of the string are always unique.
        Then we save 1 entry of hash table, and prevent 1 hash table lookup.
*/
#define APP_DATA_ID             GPOINTER_TO_UINT("PtkAppMenuItem")
...
{
    PtkAppMenuItem* data = g_object_get_qdata( G_OBJECT(a), APP_DATA_ID );
...
void  unload_old_icons( GtkWidget* menu )
{
    ...
    for( l = items; l; l = l->next )
    {
        ...
        if( ! g_object_get_qdata( G_OBJECT(l->data), APP_DATA_ID ) )
            continue;
        if( img )
                ...
                if( exec )
                {
                    ...
                    if( title )
                    {
                        ...
                        else
                        {
                            ...
                            data = (PtkAppMenuItem*)g_object_get_qdata( G_OBJECT(menu_item), APP_DATA_ID );
...
{
   return (g_object_get_qdata( G_OBJECT(item), APP_DATA_ID ) != NULL);
}
...
void ptk_app_menu_insert_items( GtkMenu* menu, int position )
{
   ...
   for( i = 0; i < G_N_ELEMENTS(known_cats); ++i )
   {
      ...
      g_object_set_qdata_full( G_OBJECT(menu_item), APP_DATA_ID, data, (GDestroyNotify) ptk_app_menu_item_free );
